...
type
   ///<summary>
   ///The Win32_Fan class represents the properties of a fan device in the computer 
   ///system. For example, the CPU cooling fan.
   ///</summary>
  ...
  public
   ...
   ///<summary>
   ///DesiredSpeed is the currently requested fan speed, defined in revolutions per 
   ///minute, when a variable speed fan is supported (VariableSpeed boolean = TRUE). 
   ///The current speed is determined via a sensor (CIM_Tachometer) that is 
   ///associated with the fan using the CIM_AssociatedSensor relationship.
   ///</summary>
   property DesiredSpeed : Int64 read FDesiredSpeed;
   ///<summary>
   ///The DeviceID property contains a string identifying the fan device.
   ///</summary>
   ...
   ///<summary>
   ///Requests that the fan speed be set to the value specified in the method's input parameter. The return value should be 0 if the request was successfully executed, 1 if the request is not supported and some other value if an error occurred. 
   ///</summary>
   function SetSpeed(DesiredSpeed : Int64): Integer;
...
{TWin32_Fan}
 ...
 constructor TWin32_Fan.Create;
 begin
   Create(True);
 end;

 constructor TWin32_Fan.Create(LoadData: boolean);
 begin
   Create(LoadData,'root\CIMV2','Win32_Fan');
 end;
 ...
 begin
    ...
    begin
       ...
       FActiveCooling                       :=VarBoolNull(GetPropertyValue('ActiveCooling'));
       ...
       FDesiredSpeed                        :=VarInt64Null(GetPropertyValue('DesiredSpeed'));
       ...
       FVariableSpeed                       :=VarBoolNull(GetPropertyValue('VariableSpeed'));
    end;
 end;


//not static, OutParams=1, InParams>0
function TWin32_Fan.SetPowerState(PowerState : Word;Time : TDateTime): Integer;
var
...
begin
  ...
  objInvoker      := objWMIService.InstancesOf('Win32_Fan');
...
//not static, OutParams=1, InParams>0
function TWin32_Fan.SetSpeed(DesiredSpeed : Int64): Integer;
var
  objSWbemLocator : OLEVariant;
  objWMIService   : OLEVariant;
  objInvoker      : OLEVariant;
...
begin
  ...
  if CurrIndex=FWmiCollectionIndex then
    begin
     ReturnValue:=colItem.SetSpeed(DesiredSpeed);
     Result     :=VarIntegerNull(ReturnValue);
     Exit;
    end;
   Inc(CurrIndex);
